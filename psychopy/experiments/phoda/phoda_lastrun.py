...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # PHODA image
    #ps_phoda_image = visual.ImageStim(
    #    win=win,
    ...
    for thisPhoda_trial in phoda_trials:
        ...
        image.setPos((0, ps_phoda_comp_img_pos_y))
        image.setSize(ps_phoda_comp_img_size)
        image.setImage(PS_PHODA_IMG_PATH)
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
        phoda_trials.addData('key_resp.keys',key_resp.keys)
        if key_resp.keys != None:  # we had a response
            phoda_trials.addData('key_resp.rt', key_resp.rt)
            phoda_trials.addData('key_resp.duration', key_resp.duration)
    ...
    intro_prompts = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('D:/Github/neuroflare-experiments/psychopy/experiments/open-closed/loopOpenClosedIntro.csv'), 
        seed=None, 
        isTrials=False, 
    )
    ...
    openclosed_trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('D:/Github/neuroflare-experiments/psychopy/experiments/open-closed/loopOpenClosedTrial.csv'), 
        seed=None, 
        isTrials=True, 
    )
