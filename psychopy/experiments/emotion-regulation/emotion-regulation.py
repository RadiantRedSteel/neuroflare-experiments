...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    aspect80 = aspect * 0.80
    comp_wrap_width_80 = aspect80   # 80% of screen width
    comp_wrap_width_20 = aspect * 0.20  # 20% of screen width
    ...
    t_body = visual.TextStim(win=win, name='t_body',
        text='',
        font='Arial',
        units='height', pos=(0.0, 0.05), draggable=False, height=0.07, wrapWidth=comp_wrap_width_80, ori=0.0, 
    ...
    t_continue = visual.TextStim(win=win, name='t_continue',
        ...
        font='Arial',
        units='height', pos=(0, -0.35), draggable=False, height=0.04, wrapWidth=comp_wrap_width_80, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    ...
    t_sm_message = visual.TextStim(win=win, name='t_sm_message',
        text='',
        font='Arial',
        pos=(0, .35), draggable=False, height=0.07, wrapWidth=comp_wrap_width_80, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    t_continue_sm = visual.TextStim(win=win, name='t_continue_sm',
        text='Press the SPACEBAR to continue',
        font='Arial',
        pos=(0, -0.35), draggable=False, height=0.04, wrapWidth=comp_wrap_width_80, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-3.0);
    # Run 'Begin Experiment' code from code_sm_helper
    # Prepare logging
    #sm_run_id = datetime.now().isoformat(timespec='seconds')
    #sm_run_id = "hello"
    #sm_out_path = Path(__file__).resolve().parent / "data" / "state_measure_responses.csv"
    #sm_out_path.parent.mkdir(parents=True, exist_ok=True)
    sm_results = []
    sm_index = 0
    ...
    for thisState_measure_pretrial in state_measure_pretrial:
        ...
        stateMeasure = data.Routine(
            name='stateMeasure',
            components=[slider_SAM, image_SAM, t_sm_message, t_continue_sm, key_resp_sm],
        )
        stateMeasure.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        slider_SAM.reset()
        slider_SAM.setPos((0, sm_sam_comp_slider_pos_y))
        slider_SAM.setSize((sm_sam_comp_slider_width, sm_sam_comp_slider_size_height))
        ...
        sm_results.append({
            #'run_id': run_id,
            'trial_index': sm_index,
            'rating_type': rating_type,
            'rating_category': rating_category,
            'rating': sm_slider.rating,
        ...
        if sm_results:
            ...
            print(f"Saved {len(results)} ratings to {out_path}")
        
        try:
            print(f"{rating_category}: {sm_slider.rating}")
        except:
    ...
    for thisRepeat in repeat:
        ...
        if thisSession is not None:
            ...
            thisSession.sendExperimentData()
        # get names of stimulus parameters
        if unpleasant_trials.trialList in ([], [None], None):
            params = []
        else:
            params = unpleasant_trials.trialList[0].keys()
        # save data for this loop
        unpleasant_trials.saveAsExcel(filename + '.xlsx', sheetName='unpleasant_trials',
            stimOut=params,
            dataOut=['n','all_mean','all_std', 'all_raw'])
