...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Github\\neuroflare-experiments\\psychopy\\open-closed_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    introPrompts = data.TrialHandler2(
        name='introPrompts',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('loop-templates/loopOpenClosedIntro.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    thisExp.addLoop(introPrompts)  # add the loop to the experiment
    thisIntroPrompt = introPrompts.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisIntroPrompt.rgb)
    if thisIntroPrompt != None:
        for paramName in thisIntroPrompt:
            globals()[paramName] = thisIntroPrompt[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisIntroPrompt in introPrompts:
        introPrompts.status = STARTED
        if hasattr(thisIntroPrompt, 'status'):
            thisIntroPrompt.status = STARTED
        currentLoop = introPrompts
        ...
        instructions = data.Routine(
            name='instructions',
            components=[tb_body, tb_continue, key_resp],
        )
        ...
        while continueRoutine:
            ...
            if hasattr(thisIntroPrompt, 'status') and thisIntroPrompt.status == STOPPING:
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
        introPrompts.addData('key_resp.keys',key_resp.keys)
        if key_resp.keys != None:  # we had a response
            introPrompts.addData('key_resp.rt', key_resp.rt)
            introPrompts.addData('key_resp.duration', key_resp.duration)
        # the Routine "instructions" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        # mark thisIntroPrompt as finished
        if hasattr(thisIntroPrompt, 'status'):
            thisIntroPrompt.status = FINISHED
        # if awaiting a pause, pause now
        if introPrompts.status == PAUSED:
    ...
    # completed 1.0 repeats of 'introPrompts'
    introPrompts.status = FINISHED
