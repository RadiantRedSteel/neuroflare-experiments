...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Github\\neuroflare-experiments\\neuroflare\\experiments\\open-closed-test\\open-closed-test_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Run 'Begin Experiment' code from code_new_state_measure
    ...
    # --- Initialize components for Routine "newStateMeasure" ---
    t_sm_continue_2 = visual.TextStim(win=win, name='t_sm_continue_2',
        ...
        languageStyle='LTR',
        depth=-1.0);
    key_resp_sm_2 = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    new_state_measure = data.TrialHandler2(
        name='new_state_measure',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
    ...
    thisExp.addLoop(new_state_measure)  # add the loop to the experiment
    thisNew_state_measure = new_state_measure.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisNew_state_measure.rgb)
    if thisNew_state_measure != None:
        for paramName in thisNew_state_measure:
            globals()[paramName] = thisNew_state_measure[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisNew_state_measure in new_state_measure:
        new_state_measure.status = STARTED
        if hasattr(thisNew_state_measure, 'status'):
            thisNew_state_measure.status = STARTED
        currentLoop = new_state_measure
        ...
        # --- Prepare to start Routine "newStateMeasure" ---
        # create an object to store info about Routine newStateMeasure
        newStateMeasure = data.Routine(
            name='newStateMeasure',
            components=[t_sm_continue_2, key_resp_sm_2],
        )
        newStateMeasure.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from code_new_state_helper
        allowContinue = False
        ...
        # create starting attributes for key_resp_sm_2
        key_resp_sm_2.keys = []
        key_resp_sm_2.rt = []
        _key_resp_sm_2_allKeys = []
        # store start times for newStateMeasure
        newStateMeasure.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        newStateMeasure.tStart = globalClock.getTime(format='float')
        newStateMeasure.status = STARTED
        thisExp.addData('newStateMeasure.started', newStateMeasure.tStart)
        newStateMeasure.maxDuration = None
        # keep track of which components have finished
        newStateMeasureComponents = newStateMeasure.components
        for thisComponent in newStateMeasure.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "newStateMeasure" ---
        thisExp.currentRoutine = newStateMeasure
        newStateMeasure.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisNew_state_measure, 'status') and thisNew_state_measure.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # Run 'Each Frame' code from code_new_state_helper
            if sm.has_rating():
                allowContinue = True
            
            # *t_sm_continue_2* updates
            
            # if t_sm_continue_2 is starting this frame...
            if t_sm_continue_2.status == NOT_STARTED and allowContinue == True:
                # keep track of start time/frame for later
                t_sm_continue_2.frameNStart = frameN  # exact frame index
                t_sm_continue_2.tStart = t  # local t and not account for scr refresh
                t_sm_continue_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(t_sm_continue_2, 'tStartRefresh')  # time at next scr refresh
                # update status
                t_sm_continue_2.status = STARTED
                t_sm_continue_2.setAutoDraw(True)
            
            # if t_sm_continue_2 is active this frame...
            if t_sm_continue_2.status == STARTED:
                # update params
                pass
            
            # *key_resp_sm_2* updates
            waitOnFlip = False
            
            # if key_resp_sm_2 is starting this frame...
            if key_resp_sm_2.status == NOT_STARTED and allowContinue == True:
                # keep track of start time/frame for later
                key_resp_sm_2.frameNStart = frameN  # exact frame index
                key_resp_sm_2.tStart = t  # local t and not account for scr refresh
                key_resp_sm_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_sm_2, 'tStartRefresh')  # time at next scr refresh
                # update status
                key_resp_sm_2.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_sm_2.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_sm_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_sm_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_sm_2.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_sm_2_allKeys.extend(theseKeys)
                if len(_key_resp_sm_2_allKeys):
                    key_resp_sm_2.keys = _key_resp_sm_2_allKeys[-1].name  # just the last key pressed
                    key_resp_sm_2.rt = _key_resp_sm_2_allKeys[-1].rt
                    key_resp_sm_2.duration = _key_resp_sm_2_allKeys[-1].duration
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=newStateMeasure,
                )
                # skip the frame we paused on
                continue
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                newStateMeasure.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if newStateMeasure.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in newStateMeasure.components:
        ...
        # --- Ending Routine "newStateMeasure" ---
        for thisComponent in newStateMeasure.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for newStateMeasure
        newStateMeasure.tStop = globalClock.getTime(format='float')
        newStateMeasure.tStopRefresh = tThisFlipGlobal
        thisExp.addData('newStateMeasure.stopped', newStateMeasure.tStop)
        # Run 'End Routine' code from code_new_state_helper
        #try:
        #    # Add data via module helper if currentLoop exists; otherwise just log
        #    end_info = None
        #    if 'currentLoop' in globals() and currentLoop is not None:
        #        end_info = sm.end_routine(currentLoop)
        #    else:
        #        end_info = sm.end_routine(None)
        #    logging.data(f"State-measure rating: {rating_category}, {end_info['rating']}")
        #except Exception:
        #    logging.error("Error printing state-measure rating")
        
        sm.end_routine(currentLoop)
        # the Routine "newStateMeasure" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        # mark thisNew_state_measure as finished
        if hasattr(thisNew_state_measure, 'status'):
            thisNew_state_measure.status = FINISHED
        # if awaiting a pause, pause now
        if new_state_measure.status == PAUSED:
    ...
    # completed 1.0 repeats of 'new_state_measure'
    new_state_measure.status = FINISHED
    
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    # get names of stimulus parameters
    if new_state_measure.trialList in ([], [None], None):
        params = []
    else:
        params = new_state_measure.trialList[0].keys()
    # save data for this loop
    new_state_measure.saveAsExcel(filename + '.xlsx', sheetName='new_state_measure',
        stimOut=params,
